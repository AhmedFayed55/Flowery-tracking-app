...
class ApplyRepoImp implements ApplyRepo {
  ...
  Future<ApiResult<void>> _validateImages(File license, File id) async {
    final resizedLicense = await _resizeImage(license);
    final resizedId = await _resizeImage(id);

    final licenseResult = await _applyRemotDataSource.verifyLicense(
      resizedLicense,
    );
    final licenseText =
        licenseResult[AppConstants.candidatesKey]?[0]?[AppConstants
            .contentKey]?[AppConstants.partsKey]?[0]?[AppConstants.textKey] ??
        "";

    if (licenseText.toLowerCase().contains(AppConstants.notAValid)) {
      return ApiErrorResult(
        failure: Failure(errorMessage: AppConstants.invalidLicenseImage),
      );
    }

    final idResult = await _applyRemotDataSource.verifyID(resizedId);
    final idText =
        idResult[AppConstants.candidatesKey]?[0]?[AppConstants
            .contentKey]?[AppConstants.partsKey]?[0]?[AppConstants.textKey] ??
        "";

    if (idText.toLowerCase().contains(AppConstants.notAValid)) {
      return ApiErrorResult(
        failure: Failure(errorMessage: AppConstants.invalidIDImage),
      );
    }
