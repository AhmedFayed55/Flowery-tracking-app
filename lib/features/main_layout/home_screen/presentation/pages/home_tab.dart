        ...
        body: BlocBuilder<HomeTabViewModel, HomeTabState>(
          builder: (context, state) {
            if (state.isLoadingGetOrders) {
              return const Expanded(child: CustomShimmerWidget());
            } else if (state.errorGetOrders != null) {
              return Center(
                child: OutlinedButton(
                  onPressed: () => getIt<HomeTabViewModel>().doIntent(
                ...
                children: [
                  Expanded(
                    child: ListView.separated(
                      itemBuilder: (context, index) => PendingOrderCart(
                        entity: state.getPendingOrdersEntity!,
                        currentIndex: index,),
