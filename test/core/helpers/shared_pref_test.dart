...
void main() {
  ...
  group('SharedPrefHelper Tests', () {
    test('saveData should call setInt when value is int', () async {
      when(mockSharedPreferences.setInt('age', 25))
    ...
    test('saveData should call setDouble when value is double', () async {
      when(mockSharedPreferences.setDouble('height', 1.85))
          .thenAnswer((_) async => true);

      final result =
          await sharedPrefHelper.saveData(key: 'height', val: 1.85);
    ...
    test('saveData should call setString when value is String', () async {
      when(mockSharedPreferences.setString('name', 'Ahmed'))
          .thenAnswer((_) async => true);

      final result =
          await sharedPrefHelper.saveData(key: 'name', val: 'Ahmed');
    ...
    test('saveData should call setBool when value is bool', () async {
      ...
      final result =
          await sharedPrefHelper.saveData(key: 'isLoggedIn', val: true);
