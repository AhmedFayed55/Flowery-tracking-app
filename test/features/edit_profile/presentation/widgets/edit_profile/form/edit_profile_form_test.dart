...
void main() {
  testWidgets('EditProfileForm dispatches LoadUserData on init and submit on tap', (
    tester,
  ) async {
    ...
    when(mockVm.editProfileEmailController).thenReturn(TextEditingController());
    when(mockVm.editProfilePhoneController).thenReturn(TextEditingController());
    ...
    // initState should have dispatched LoadUserDataEvent
    verify(mockVm.doIntend(argThat(isA<LoadUserDataEvent>()))).called(1);

    // Tap the update button and expect submit intent
    // final updateFinder = find.widgetWithText(
    //   CustomElevatedButton,
    //   AppLocalizations.of(tester.element(find.byType(EditProfileForm)))!.update,
    // );
    ...
    // verify(mockVm.doIntend(argThat(isA<EditProfileSubmitEvent>()))).called(1);
  });
}
